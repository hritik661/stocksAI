...
export function getStockLogoDataUrl(symbol: string | undefined, name?: string, size = 64) {
  const label = initials(name || symbol || "?")
  const bg = hashStringToColor((symbol || name || "seed") as string)
...
export function getStockLogoAttemptUrl(symbol: string | undefined, name?: string, size = 64) {
  if (!symbol) {
    return getStockLogoDataUrl("?", name, size)
  }
...
export function getStockLogoCandidates(symbol: string | undefined, name?: string, size = 64) {
  ...
  const KNOWN_EXT: Record<string, string> = {
    ...
    "PETRONET.NS": "petronetlng.com",
    "GUJARATGAS.NS": "gujaratgasltd.com",
    ...
    "NYKAA.NS": "nykaa.com",
    "DMART.NS": "dmart.in",
    "AVENUE.NS": "avenuesupermarts.com",
    "TRENT.NS": "trentlimited.com",
