...
export async function POST(request: NextRequest) {
  try {
    ...
    if (currentBalance < amount) {
      return NextResponse.json(
        { success: false, error: `Insufficient balance. Available: ₹${currentBalance}` },
        { status: 400 }
      )
    }
    ...
    console.log(`[v0] Balance deducted for ${email}: ₹${amount}, new balance: ₹${newBalance}`)

    return NextResponse.json(
      {
        success: true,
        message: `Transaction successful. ₹${amount} deducted.`,
        newBalance,
      },
      { status: 200 }
    )
